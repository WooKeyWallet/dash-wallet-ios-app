...
class Helper: NSObject {
    ...
    class func priceForDash(_ amount: UInt64) -> String {
        let symbol = WKDefaults.shared.currencyCode == "CNY" ? "¥" : "$"
        guard amount != UInt64.max, amount != Int64.max else {
            return "≈ \(symbol)0"
        }
        let priceManager = DSPriceManager.sharedInstance()
        guard let priceNum = priceManager.localCurrencyDashPrice else {
            return "≈ \(symbol)--"
        }
        let decimal = priceManager.number(forAmount: Int64(amount)).decimalValue * priceNum.decimalValue
        return "≈ \(symbol)\(decimal.scaleString(2))"
    }
    
    class func amountForDash(_ balance: Int64) -> String {
        let priceManager = DSPriceManager.sharedInstance()
        if let str = priceManager.string(forDashAmount: balance) {
            let _s = str.replacingOccurrences(of: "DASH", with: "")
            return String(_s.suffix(_s.count - 1))
        }
        return ""
    }
}
