...
class TransactionDetailBasicInfoView: UIView {
    ...
    internal func configureConstraints() {
        stride(from: 0, to: 5, by: 1).forEach { (i) in
            ...
            titleLabel.snp.makeConstraints({ (make) in
                ...
                make.right.equalTo(contentLabel.snp.left).offset(-12)
            })
            if i == 0 {
                contentLabel.snp.makeConstraints({ (make) in
                    make.top.right.equalToSuperview()
                    make.left.equalTo(contentLabels.last!)
    ...
    public func configureContents(attributeStrs: [NSAttributedString]) {
        stride(from: 0, to: min(attributeStrs.count, contentLabels.count), by: 1).forEach { (i) in
            contentLabels[i].attributedText = attributeStrs[i]
        }
    }
...
class TransactionDetailView: UIView {
    ...
    func configure(model: Transaction) {
        ...
        switch model.type {
        case .in:
            amount = model.amount; amountPrice = model.amountPrice
            direction = LocalizedString(key: "receive", comment: "")
        case .out:
            amount = model.receiveAmount; amountPrice = model.receiveAmountPrice
            direction = LocalizedString(key: "send", comment: "")
        default:
            break
        }
        baseInfoView.configureTitles(strs: [
            ...
            LocalizedString(key: "receiveAddress", comment: ""),
            LocalizedString(key: "sendAddress", comment: ""),
        ])
        let addPriceAttribute = {
            (amount: String, price: String) -> NSAttributedString in
            let amountStr = Helper.displayDigitsAmount(amount) + " DASH "
            let priceStr = Helper.displayDigitsAmount(price)
            return amountStr.attributeText().esay_append(priceStr.attributeText([.foregroundColor: AppTheme.Color.text_light]))
        }
        baseInfoView.configureContents(attributeStrs: [
            addPriceAttribute(amount, amountPrice),
            addPriceAttribute(model.fee, model.feePrice),
            direction.attributeText(),
            model.receiveAddress.attributeText(),
            model.sendAddress.attributeText(),
        ])
