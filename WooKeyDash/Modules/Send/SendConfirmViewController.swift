...
class SendConfirmViewController: BaseViewController {
    ...
    required init(viewModel: SendViewModel, model: SendModel) {
    ...
    override func configureBinds() {
        ...
        /// ViewModel ->
        viewModel.configureConfirm(confirmView, sendModel: send)
        
        viewModel.popState.observe(self) { (offset, _Self) in
            _Self.navigationController?.popTo(offset)
        }
    }
    ...
    @objc private func confirmAction() {
        WKAuthenticator.shared.request {
        [unowned self] (success) in
            guard success else { return }
            self.viewModel.commitTx(send: self.send)
        }
    }
