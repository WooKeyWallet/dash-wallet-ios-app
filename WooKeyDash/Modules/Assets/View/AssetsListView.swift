...
class AssetsTokenItemView: UIView {
    ...
    public var balanceSecureTextEntry: Bool = WKDefaults.shared.hiddenAsset {
        didSet {
            remainLabel.text = balanceSecureTextEntry ? "****" : balance
            priceLabel.text = balanceSecureTextEntry ? "****" : price
        }
    }
    ...
    override func layoutSubviews() {
        ...
        let remainH = remainLabel.font.lineHeight
        let priceH = priceLabel.font.lineHeight
        remainLabel.updateFrame(CGRect(x: tokenNameLabel.maxX + 10, y: midY - (remainH + priceH + 2) * 0.5, width: rightArrow.x - tokenNameLabel.maxX - 20, height: remainH))
        priceLabel.updateFrame(CGRect(x: remainLabel.x, y: remainLabel.maxY + 2, width: remainLabel.width, height: priceH))
    }
    ...
    private func configureView() {
        ...
        do///Subviews
        {
            ...
            priceLabel.textColor = AppTheme.Color.text_light
            priceLabel.font = AppTheme.Font.text_small
...
class AssetsListView: UIView {
    ...
    public var balanceSecureTextEntryState = WKDefaults.shared.hiddenAsset {
        didSet {
            updateBalance()
        }
    }
