...
class SettingsViewModel: NSObject {
    ...
    private func getItemFor(_ type: SettingsItemType) -> SettingsItem {
        ...
        case .currency:
            return SettingsItem.init(icon: UIImage(named: "settings.currency"), name: LocalizedString(key: "currency.exechange.rate", comment: ""), text: WKDefaults.shared.currencyCode)
        case .lang:
            let text: String
    ...
    public func getNextViewController(_ indexPath: IndexPath) -> UIViewController {
        ...
        case .currency:
            return CurrencyForDashAmountController(viewModel: self)
        case .lang:
    ...
    public func reloadData() {
        DispatchQueue.global().async {
            self.dataSourceState.newState(self.getDataSource())
        }
    }
}
